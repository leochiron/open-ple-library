...
echo '<pre style="background:#f0f0f0; padding:20px; margin:20px; border:1px solid red;">';
echo "=== DEBUG INFO ===\n";
echo "REQUEST_URI: " . ($_SERVER['REQUEST_URI'] ?? 'NONE') . "\n";
echo "GET params: " . var_export($_GET, true) . "\n";
...
try {
    ...
    echo '<pre style="background:#ffffcc; padding:20px; margin:20px; border:1px solid orange;">';
    echo "Extracted from REQUEST_URI:\n";
    echo "Path (before decode): " . var_export($requestUri, true) . "\n";
    echo "Path (after decode): " . var_export($path, true) . "\n";
    echo "Action: " . var_export($action, true) . "\n";
    echo "</pre>\n";
    
    $path = $security->validateRelativePath($path);
    
    echo '<pre style="background:#ccffcc; padding:20px; margin:20px; border:1px solid green;">';
    echo "Validated path: " . var_export($path, true) . "\n";
    echo "</pre>\n";
