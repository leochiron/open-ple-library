...
<header class="app-header">
    ...
    <?php
        $brandLabel = $config['branding']['site_label'] ?? $config['branding']['site_name'] ?? 'Cours de Léo Chiron';
        $brandLogo = $config['branding']['site_logo'] ?? '';
        $useLogo = $brandLogo !== '';
    ?>
    <div class="brand">
        <a href="<?php echo htmlspecialchars(buildUrl(['path' => '']), ENT_QUOTES, 'UTF-8'); ?>" class="brand-link">
            <?php if ($useLogo): ?>
                <img src="<?php echo htmlspecialchars($brandLogo, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($config['app_name'], ENT_QUOTES, 'UTF-8'); ?>" class="logo">
            <?php else: ?>
                <span class="brand-text"><?php echo htmlspecialchars($brandLabel, ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endif; ?>
        </a>
    </div>
</header>
